                  ...
                  <TableHead>
                        <TableRow>
                            <TableCell sx={{borderWidth:'0px', padding:'0px', width:'30px'}}>
                                <Checkbox sx={{ '&.MuiCheckbox-root': {color: '#4C4C4C'}}}
                        ...
                        {visibleRows.map((row, index) => {
                            ...
                            >
                                ...
                                <TableCell sx={{borderWidth:'0px', padding:'12px'}}
                                            id={labelId} scope="row" padding="none"  > {row.name} </TableCell>
                                <TableCell sx={{borderWidth:'0px', padding:'12px'}}> {row.org}  </TableCell>
                                <TableCell sx={{borderWidth:'0px', padding:'12px'}}> {row.role} </TableCell>
                                <TableCell sx={{borderWidth:'0px', padding:'12px', width:'180px'}}> {row.date} </TableCell>
                            </TableRow>
                            );
                        })}
                        {emptyRows > 0 && ( <TableRow style={{ height: 45 * emptyRows, }}> <TableCell colSpan={6} /></TableRow> )}
                    </TableBody>
                </Table>
            </TableContainer>
            <TablePagination
...
function getUserDataFromBackend():IUserData[] {
    ...
    function createUserData(id:number, name: string, org: string, role: string, date: string) {
...
export function Users() {
    ...
    return ( <>
        <Box >
            {/* кнопка "добавить" */}
            <Box display={'flex'} flexDirection={'row'} alignItems={'center'} gap={'18px'} marginBottom={'18px'}>
                <Fab color="primary" aria-label="add" sx={{'&&.MuiFab-root ': { bgcolor:'#4C4C4C' }, '&&.MuiFab-root:hover' : { bgcolor:'#6C6C6C' }}}>
                    <AddIcon />
                </Fab>
                ...
                <Toolbar sx={{display:'flex', flexDirection:'row', justifyContent:'space-between', bgcolor:'white', '&.MuiToolbar-root':{padding:'5px'}}}>
                    ...
                    <Box sx={{display:'flex', flexDirection:'row', alignItems:'center'}}>
                        ...
                        <Tooltip title='Изменить выбранное' placement='top'>
                            <span><IconButton disabled={disableEdit}><EditIcon /></IconButton></span>
                        </Tooltip>
                        <Tooltip title='Удалить выбранное' placement='top'>
                            <span><IconButton disabled={disableEdit}><DeleteIcon/></IconButton></span>
                        </Tooltip>
                        <Tooltip title='Обновить данные' placement='top'>
                            <span><IconButton><SyncIcon /*sx={{color:'#4C4C4C'}}*//></IconButton></span>
                        </Tooltip>
                    </Box>
                </Toolbar>
